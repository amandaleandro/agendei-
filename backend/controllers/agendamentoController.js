...
const { enviarWhatsApp } = require('./notificationsController');
            ...
            novoAgendamento.save((err) => {
                if (err) {
                    ...
                    bloquearHorario(profissionalId, data, hora_inicio, hora_fim, () => {
                        enviarWhatsApp(clienteId, 'Seu serviço foi agendado com sucesso!');
                        agendarLembretesWhatsApp(clienteId, data, hora_inicio);
                        ...
                        if (err) {
                            ...
                            reembolsarPagamento(agendamento.pagamento_id, (resultado) => {
                                if (resultado.success) {
                                    enviarWhatsApp(agendamento.cliente_id, 'Seu serviço foi cancelado e o pagamento foi estornado.');
...
function agendarLembretesWhatsApp(clienteId, data, hora_inicio) {
    ...
    cron.schedule(`0 9 ${umaSemanaAntes.getDate()} ${umaSemanaAntes.getMonth() + 1} *`, () => {
        enviarWhatsApp(clienteId, 'Lembrete: Seu serviço está agendado para daqui uma semana.');
    });
    ...
    cron.schedule(`0 9 ${doisDiasAntes.getDate()} ${doisDiasAntes.getMonth() + 1} *`, () => {
        enviarWhatsApp(clienteId, 'Lembrete: Seu serviço está agendado para daqui dois dias.');
    });

    // Lembrete no dia do serviço
    cron.schedule(`0 9 ${dataAgendamento.getDate()} ${dataAgendamento.getMonth() + 1} *`, () => {
        enviarWhatsApp(clienteId, 'Lembrete: Seu serviço está agendado para hoje.');
    });
    ...
    cron.schedule(`${umaHoraAntes.getMinutes()} ${umaHoraAntes.getHours()} ${umaHoraAntes.getDate()} ${umaHoraAntes.getMonth() + 1} *`, () => {
        enviarWhatsApp(clienteId, 'Lembrete: Seu serviço está agendado para daqui uma hora.');
    });
}
